...
async function loadData() {
  ...
  try {
    ...
    notices.value = res.records; total.value = res.total
  } catch {} finally { loading.value = false }
}
function openAddDialog() { isEdit.value = false; form.value = { title: '', content: '', type: 'notice', status: 1 }; dialogVisible.value = true }
function openEditDialog(item: Notice) { isEdit.value = true; form.value = { ...item }; dialogVisible.value = true }
async function submitForm() {
  if (!form.value.title) { ElMessage.warning('请输入公告标题'); return }
  try {
    if (isEdit.value) { await updateNotice(form.value); ElMessage.success('更新成功') }
    else { await addNotice(form.value); ElMessage.success('发布成功') }
    dialogVisible.value = false; await loadData()
  } catch {}
}
async function handleDelete(item: Notice) {
  try {
    await ElMessageBox.confirm(`确定删除公告「${item.title}」吗？`, '确认删除', { type: 'warning', confirmButtonText: '删除', cancelButtonText: '取消' })
    await deleteNotice(item.id); ElMessage.success('删除成功'); await loadData()
  } catch {}
}
  ...
  <div>
    ...
    <el-card>
        ...
        <div class="card-header">
          <div class="header-left">
            ...
            <el-select v-model="query.type" placeholder="公告类型" clearable size="small" style="width:120px;margin-left:16px" @change="loadData">
              <el-option label="全部" value="" /><el-option label="系统公告" value="notice" /><el-option label="活动公告" value="activity" /><el-option label="更新日志" value="update" />
            </el-select>
          </div>
      ...
      <el-table :data="notices" v-loading="loading" stripe style="width:100%">
        ...
        <el-table-column prop="type" label="类型" width="110"><template #default="{ row }"><el-tag :type="row.type === 'notice' ? '' : row.type === 'activity' ? 'warning' : 'info'" size="small">{{ row.type === 'notice' ? '系统公告' : row.type === 'activity' ? '活动公告' : '更新日志' }}</el-tag></template></el-table-column>
        <el-table-column prop="status" label="状态" width="80"><template #default="{ row }"><el-tag :type="row.status === 1 ? 'success' : 'info'" size="small">{{ row.status === 1 ? '发布' : '草稿' }}</el-tag></template></el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑公告' : '发布公告'" width="650px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="类型"><el-select v-model="form.type" style="width:160px"><el-option label="系统公告" value="notice" /><el-option label="活动公告" value="activity" /><el-option label="更新日志" value="update" /></el-select></el-form-item>
        <el-form-item label="内容"><el-input v-model="form.content" type="textarea" :rows="6" /></el-form-item>
        <el-form-item label="状态"><el-radio-group v-model="form.status"><el-radio :value="1">发布</el-radio><el-radio :value="0">草稿</el-radio></el-radio-group></el-form-item>
      </el-form>
