...
const form = ref<Partial<Product>>({ name: '', categoryId: 0, price: 0, originalPrice: 0, stock: 0, tag: '', description: '', mainImage: '', status: 1, isRecommend: 0, sales: 0 })
...
function openAddDialog() {
  isEdit.value = false
  form.value = { name: '', categoryId: 0, price: 0, originalPrice: 0, stock: 0, tag: '', description: '', mainImage: '', status: 1, isRecommend: 0, sales: 0 }
  ...
  <div>
    ...
    <el-card>
      ...
      <el-table :data="products" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column prop="price" label="价格" width="90"><template #default="{ row }">¥{{ row.price }}</template></el-table-column>
        <el-table-column prop="stock" label="库存" width="70" />
        <el-table-column prop="sales" label="销量" width="70" />
        <el-table-column prop="status" label="状态" width="80"><template #default="{ row }"><el-tag :type="row.status === 1 ? 'success' : 'danger'" size="small">{{ row.status === 1 ? '上架' : '下架' }}</el-tag></template></el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑商品' : '新增商品'" width="650px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="分类"><el-select v-model="form.categoryId" style="width:100%"><el-option v-for="cat in categories" :key="cat.id" :label="cat.name" :value="cat.id" /></el-select></el-form-item>
        <el-row :gutter="16">
        ...
        <el-form-item label="标签"><el-input v-model="form.tag" style="width:240px" placeholder="新品/热销" /></el-form-item>
