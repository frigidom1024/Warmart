...
const form = ref<Partial<Category>>({
  name: '', parentId: 0, sort: 0, imageUrl: '', status: 1
})
...
function openAddDialog(parentId?: number) {
  isEdit.value = false
  form.value = { name: '', parentId: parentId || 0, sort: 0, imageUrl: '', status: 1 }
...
async function handleDelete(item: Category) {
  try {
    await ElMessageBox.confirm(`确定删除分类「${item.name}」吗？`, '确认删除', {
...
function getStatusTag(status: number | null | undefined) {
  return status === 0 ? { type: 'danger', text: '禁用' } : { type: 'success', text: '启用' }
}
      ...
      <el-table :data="treeData" v-loading="loading" stripe row-key="id" style="width: 100%">
        <el-table-column prop="name" label="分类名称" min-width="200">
          <template #default="{ row }">
            <span :style="{ paddingLeft: row.parentId && row.parentId !== 0 ? '24px' : '0', fontWeight: !row.parentId || row.parentId === 0 ? 600 : 400 }">
              {{ row.name }}
            </span>
          </template>
        </el-table-column>
      ...
      <el-form ref="formRef" :model="form" label-width="80px">
        ...
        <el-form-item label="父级分类">
          <el-select v-model="form.parentId" placeholder="顶级分类" clearable>
            ...
            <el-option
              v-for="item in categories.filter(c => !c.parentId || c.parentId === 0)"
