...
async function loadDashboard() {
  ...
  try {
    ...
    const todayOrders = allOrders.filter((o: Order) =>
      o.createdTime && o.createdTime.slice(0, 10) === today
    )
  ...
  try {
    ...
    totalUsers.value = productRes.total || 0
  } catch {}
        ...
        <el-card shadow="hover" class="stat-card">
          <div class="stat-content">
            ...
            <div class="stat-icon order-icon">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
            </div>
          </div>
        </el-card>
      </el-col>
...
.page-desc { font-size: 13px; color: var(--wa-text-secondary, #909399); }
...
.stat-info { flex: 1; }
.stat-label { margin: 0 0 4px; font-size: 13px; color: var(--wa-text-secondary, #909399); }
.stat-value { margin: 0 0 6px; font-size: 26px; font-weight: 700; color: var(--wa-text-primary, #303133); }
.stat-icon { width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.order-icon { background: #ecf5ff; color: var(--wa-color-primary, #409eff); }
