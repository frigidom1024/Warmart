...
async function loadData() {
  loading.value = true
  try {
    feedbacks.value = await getFeedbackList()
  } catch {} finally {
    loading.value = false
  }
}

function openReplyDialog(item: Feedback) {
  currentItem.value = item
  replyForm.value.replyContent = item.reply || ''
  replyDialogVisible.value = true
}

async function submitReply() {
  if (!currentItem.value || !replyForm.value.replyContent) {
...
const typeLabels: Record<string, string> = {
  suggestion: '建议', complaint: '投诉', question: '咨询', other: '其他'
}

const typeColors: Record<string, string> = {
  suggestion: '', complaint: 'danger', question: 'primary', other: 'info'
}
      ...
      <el-table :data="feedbacks" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column prop="type" label="类型" width="100">
          <template #default="{ row }">
            <el-tag :type="typeColors[row.type] || ''" size="small">
...
.original-feedback p { margin: 6px 0 0; font-size: 14px; color: #303133; line-height: 1.6; }
