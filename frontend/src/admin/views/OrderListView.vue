...
async function handleUpdateStatus(order: Order, newStatus: number, label: string) {
  try {
    await ElMessageBox.confirm(`确定将订单 ${order.orderNo} 设为「${label}」吗？`, '确认操作', {
  ...
  <div>
    ...
    <el-card>
      ...
      >
            ...
            <div class="order-items">
              <div v-for="item in row.items" :key="item.id" class="order-item">
                ...
                <div class="item-info">
                  ...
                  <p class="item-spec" v-if="item.specInfo">{{ item.specInfo }}</p>
                </div>
        ...
        <el-table-column prop="receiverName" label="收货人" width="120" show-overflow-tooltip />
        <el-table-column prop="receiverPhone" label="电话" width="120" />
        <el-table-column prop="receiverAddress" label="地址" min-width="180" show-overflow-tooltip />
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              ...
              @click="handleUpdateStatus(row, 2, '待收货')"
            >发货</el-button>
            <el-button
              v-if="row.status === 2"
              size="small" type="success"
              @click="handleUpdateStatus(row, 3, '已完成')"
            >确认收货</el-button>
          </template>
        </el-table-column>
      </el-table>
